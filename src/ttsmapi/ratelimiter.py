...
class Store:
    ...
    def __init__(self, persist: bool=False) -> None:
        ...
        Raises:
            RuntimeError: If the shelve file cannot be opened.

        """
        self.dict: shelve.Shelf|dict = {}
        if persist:
            try:
                self.dict = shelve.open('.ratelimiter_store')  # noqa: SIM115
            except (OSError, KeyError, PicklingError, UnpicklingError) as e:
                if isinstance(self.dict, shelve.Shelf):
                    self.dict.close()
                raise RuntimeError("Failed to open shelve file for rate limiter store") from e

    def __del__(self) -> None:
        """Ensure the shelve file is closed if used."""
        if isinstance(self.dict, shelve.Shelf):
            self.dict.close()
